...
class Gold:
    ...
    def run(self):
        # create invoice_gl table
        spark.sql("create table if not exists invoice_gl (customer_id string, total_amount double) using delta")
    ...
    def upsert(self, df, epoch_id):
        df.createOrReplaceTempView("invoice_gl_tmp_view")

        merge_stm = """
        MERGE INTO invoice_gl AS target
        USING invoice_gl_tmp_view AS source
        ...
        df._jdf.sparkSession().sql(merge_stm)

# clean up
spark.sql("drop table if exists invoice_bz")
spark.sql("drop table if exists invoice_gl")
